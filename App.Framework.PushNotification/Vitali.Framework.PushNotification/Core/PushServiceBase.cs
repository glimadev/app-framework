...
namespace Vitali.Framework.PushNotification.Core
{
	...
	public delegate void NotificationSentDelegate(object sender, INotification notification, ResultPush resultPush);
	public delegate void NotificationFailedDelegate(object sender, INotification notification, Exception error, ResultPush resultPush);
	...
	public abstract class PushServiceBase : IPusherService
	{
		                               ...
		                               bool ignoreStoppingChannel = false, bool queueToFront = false)
		{
			...
			else
			{
				...
				if (evt != null)
					evt(this, notification, new MaxSendAttemptsReachedException(), resultPush);
		...
		private void DoChannelWork(IPushChannel channel, CancellationTokenSource cancelTokenSource)
		{
			...
			while (!cancelTokenSource.IsCancellationRequested)
			{
						...
						else
						{
							...
							if (!result.IsSuccess)
							{
								...
								else //Otherwise some general failure
								{
									var evt = this.OnNotificationFailed;
									if (evt != null)
                                        evt(this, result.Notification, result.Error, resultPush);
								}
							}
							else
							{
								...
								if (evt != null)
									evt(this, result.Notification, resultPush);
							}
						}
					});
                ...
                if (waitForNotification != null && !waitForNotification.WaitOne(ServiceSettings.NotificationSendTimeout))
                {
					...
					var evt = this.OnNotificationFailed;
					if (evt != null)
                        evt(this, notification, new TimeoutException("Notification send timed out"), resultPush);
				}
